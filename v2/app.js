// Học Tiếng Anh — Flashcard theo chủ đề — app logic
// Globals from data.js: window.data = { days: [{ id, theme, themeEn, cards: [...] }] }
...
(function () {
  ...
  // Chống lỗi state cũ: nếu currentDay vượt số chủ đề hiện có thì đưa về 1.
  if (window.data && window.data.days &&
      (state.currentDay < 1 || state.currentDay > window.data.days.length)) {
    state.currentDay = 1;
  ...
  function buildPickerGrid() {
    // Lưới phẳng tất cả chủ đề (không phân lớp); mỗi ô là một nút tên chủ đề.
